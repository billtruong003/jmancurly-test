...
using Fusion; // Đã thêm using Fusion
using System.Collections.Generic;
using UnityEngine;

namespace Synty.AnimationBaseLocomotion.Samples
{
    public class SamplePlayerAnimationController : NetworkBehaviour // Kế thừa NetworkBehaviour
    {
        // Define NetworkInputData struct for network input synchronization
        ...
        public enum EInputButtons : byte // Important: Use byte for NetworkButtons and define in SamplePlayerAnimationController for clarity
        {
            Jump = 0,
            Sprint = 1,
            Crouch = 2,
        ...
        private float _movementInputDuration; // Added to fix the error

        [Networked, OnChangedRender(nameof(OnNetworkedSpeed2DChanged))] // Networked Speed2D (MoveSpeed)
        public float _networkedSpeed2D { get; set; }

        private float _speed2D; // Biến cục bộ để animation sử dụng, được cập nhật từ _networkedSpeed2D
        ...
        [Networked, OnChangedRender(nameof(OnNetworkedMoveDirectionChanged))] // Networked MovementDirection
        public Vector2 _networkedMoveDirection { get; set; }

        private Vector3 _moveDirection = Vector3.zero; // Biến cục bộ để animation sử dụng, được cập nhật từ _networkedMoveDirection

        private Vector3 _previousRotation;
        private Vector3 _velocity;

        [Networked, OnChangedRender(nameof(OnNetworkedIsJumpingChanged))] // Networked IsJumping
        public bool _networkedIsJumping { get; set; }

        private bool _isJumping; // Biến cục bộ để animation sử dụng, được cập nhật từ _networkedIsJumping

        [Networked, OnChangedRender(nameof(OnNetworkedIsCrouchingChanged))] // Networked IsCrouching
        public bool _networkedIsCrouching { get; set; }

        private bool _isCrouching; // Biến cục bộ để animation sử dụng, được cập nhật từ _networkedIsCrouching

        [Networked] public NetworkButtons PreviousButtons { get; set; } // Networked previous button state - ADDED
        ...
        public override void Spawned() // Thay Start bằng Spawned và override nó
        {
        ...
        /// </summary>
        public void ToggleWalk() // Made public to be called from external input
        {
        ...
        private void ActivateCrouch()
        {
            ...
            if (_isGrounded)
            {
                ...
                DeactivateSprint();
                _networkedIsCrouching = true; // Cập nhật Networked Property
            }
        }
        ...
        private void DeactivateCrouch()
        {
            ...
            if (!_cannotStandUp && !_isSliding)
            {
                ...
                _networkedIsCrouching = false; // Cập nhật Networked Property
            }
        }
        ...
        public override void FixedUpdateNetwork() // Changed Update to FixedUpdateNetwork for Fusion
        {
            if (GetInput<NetworkInputData>(out NetworkInputData inputData))
            {
                ProcessInput(inputData); // Process input here
            }
        ...
        {
            _networkedMoveDirection = inputData.MoveDirection; // Set networked move direction from input
            _moveDirection = new Vector3(_networkedMoveDirection.x, 0, _networkedMoveDirection.y); // Update local move direction

            NetworkButtons pressed = inputData.Buttons.GetPressed(PreviousButtons); // Get buttons pressed this frame
            NetworkButtons released = inputData.Buttons.GetReleased(PreviousButtons); // Get buttons released this frame

            // Store current buttons as previous for next frame comparison
            PreviousButtons = inputData.Buttons;

            // Jump (check for pressed event using GetPressed)
            if (pressed.IsSet(EInputButtons.Jump))
            {
                if (_isGrounded && _currentState != AnimationState.Jump)
                {
                    LocomotionToJumpState();
                }
            }

            // Sprint (check for continuous press using IsSet)
            if (inputData.Buttons.IsSet(EInputButtons.Sprint))
            {
                if (!_isSprinting && _isGrounded && !_isCrouching)
                {
                    ActivateSprint();
                }
            }
            ...
            // Crouch (toggle on press using GetPressed)
            if (pressed.IsSet(EInputButtons.Crouch))
            {
                if (!_isCrouching)
                {
                    ActivateCrouch();
            ...
            // Aim (check for continuous press using IsSet)
            if (inputData.Buttons.IsSet(EInputButtons.Aim))
            {
                if (!_isAiming)
                {
                    ActivateAim();
            ...
            // LockOn (toggle on press using GetPressed)
            if (pressed.IsSet(EInputButtons.LockOn))
            {
                ToggleLockOn();
            }

            // Toggle Walk (toggle on press using GetPressed)
            if (pressed.IsSet(EInputButtons.ToggleWalk))
            {
                ToggleWalk();
            }
        }
        ...
        {
            if (_moveDirection.magnitude > 0) // Check moveDirection magnitude instead of inputReader
            {
                ...
                _movementInputDuration += Runner.DeltaTime; // Use Runner.DeltaTime
            }
            else
            {
            ...
            _moveDirection = (_cameraController.GetCameraForwardZeroedYNormalised() * _networkedMoveDirection.y) // Using networked move direction
                + (_cameraController.GetCameraRightZeroedYNormalised() * _networkedMoveDirection.x);
        }
        ...
        private void Move()
        {
            _controller.Move(_velocity * Runner.DeltaTime); // Use Runner.DeltaTime for Fusion
        ...
        private void ApplyGravity()
        {
            ...
            {
                _velocity.y += Physics.gravity.y * _gravityMultiplier * Runner.DeltaTime; // Use Runner.DeltaTime
            }
        }
        ...
        private void CalculateMoveDirection()
        {
            ...
            _currentMaxSpeed = Mathf.Lerp(_currentMaxSpeed, _targetMaxSpeed, _speedChangeDamping * Runner.DeltaTime); // Use Runner.DeltaTime

            _targetVelocity.x = _moveDirection.x * _currentMaxSpeed;
            _targetVelocity.z = _moveDirection.z * _currentMaxSpeed;

            _velocity.z = Mathf.Lerp(_velocity.z, _targetVelocity.z, _speedChangeDamping * Runner.DeltaTime); // Use Runner.DeltaTime
            _velocity.x = Mathf.Lerp(_velocity.x, _targetVelocity.x, _speedChangeDamping * Runner.DeltaTime); // Use Runner.DeltaTime

            _networkedSpeed2D = new Vector3(_velocity.x, 0f, _velocity.z).magnitude; // Cập nhật Networked Speed2D
            _speed2D = _networkedSpeed2D; // Cập nhật biến cục bộ _speed2D
        ...
        private void FaceMoveDirection()
        {
            ...
            if (_isStrafing)
            {
                if (_moveDirection.magnitude > 0.01)
                {
                    if (_cameraForward != Vector3.zero)
                    {
                        ...
                        _cameraRotationOffset = Mathf.Lerp(_cameraRotationOffset, 0f, _rotationSmoothing * Runner.DeltaTime); // Use Runner.DeltaTime
                        ...
                        {
                            float t = Mathf.Clamp01(_STRAFE_DIRECTION_DAMP_TIME * Runner.DeltaTime); // Use Runner.DeltaTime
                            _forwardStrafe = Mathf.SmoothStep(_forwardStrafe, targetValue, t);
                        }
                    }

                    transform.rotation = Quaternion.Slerp(transform.rotation, strafingTargetRotation, _rotationSmoothing * Runner.DeltaTime); // Use Runner.DeltaTime
                }
                else
                {
                    UpdateStrafeDirection(1f, 0f);

                    float t = 20 * Runner.DeltaTime; // Use Runner.DeltaTime
                    float newOffset = 0f;
            ...
            else
            {
                ...
                transform.rotation = Quaternion.Slerp(
                    ...
                    _rotationSmoothing * Runner.DeltaTime // Use Runner.DeltaTime
                );
            }
        }
        ...
        {
            _strafeDirectionZ = Mathf.Lerp(_strafeDirectionZ, TargetZ, _ANIMATION_DAMP_TIME * Runner.DeltaTime); // Use Runner.DeltaTime
            _strafeDirectionX = Mathf.Lerp(_strafeDirectionX, TargetX, _ANIMATION_DAMP_TIME * Runner.DeltaTime); // Use Runner.DeltaTime
        ...
        private void GroundInclineCheck()
        {
            ...
            _inclineAngle = Mathf.Lerp(_inclineAngle, Mathf.Atan2(hitDifference.y, xPlaneLength) * Mathf.Rad2Deg, 20f * Runner.DeltaTime); // Use Runner.DeltaTime
        }
        ...
        {
            _fallStartTime = Runner.SimulationTime; // Use Runner.SimulationTime for Fusion
            _fallingDuration = 0f;
        }
        ...
        {
            _fallingDuration = Runner.SimulationTime - _fallStartTime; // Use Runner.SimulationTime
        }
        ...
        private void CalculateRotationalAdditives(bool leansActivated, bool headLookActivated, bool bodyLookActivated)
        {
            if (headLookActivated || leansActivated || bodyLookActivated)
            {
                ...
                _rotationRate = _currentRotation != _previousRotation
                    ? Vector3.SignedAngle(_currentRotation, _previousRotation, Vector3.up) / Runner.DeltaTime * -1f // Use Runner.DeltaTime
                    : 0f;
            }
            ...
            if (headLookActivated && _isTurningInPlace)
            {
                ...
                _headLookX = Mathf.Lerp(_headLookX, _initialTurnValue / 200, 5f * Runner.DeltaTime); // Use Runner.DeltaTime
            }
            else
            {
        ...
        )
        {
            ...
            {
                changeVariable = Mathf.Lerp(mainVariable, changeVariable, smoothness * Runner.DeltaTime); // Use Runner.DeltaTime
            }
        ...
        private float VariableOverrideDelayTimer(float timeVariable)
        {
            ...
            {
                timeVariable -= Runner.DeltaTime; // Use Runner.DeltaTime
        ...
        private void EnterJumpState()
        {
            ...
            _networkedIsJumping = true; // Cập nhật Networked Property khi bắt đầu nhảy
        }
        ...
        private void UpdateJumpState()
        {
            ...
            if (_velocity.y <= 0f)
            {
                ...
                _networkedIsJumping = false; // Cập nhật Networked Property khi hết nhảy
                SwitchState(AnimationState.Fall);
            }
        ...
        void OnNetworkedIsCrouchingChanged() { _isCrouching = _networkedIsCrouching; _animator.SetBool(_isCrouchingHash, _isCrouching); } // Cập nhật animator trực tiếp ở đây
        void OnNetworkedIsJumpingChanged() { _isJumping = _networkedIsJumping; _animator.SetBool(_isJumpingAnimHash, _isJumping); } // Cập nhật animator trực tiếp ở đây
        void OnNetworkedSpeed2DChanged() { _speed2D = _networkedSpeed2D; }
        void OnNetworkedMoveDirectionChanged() { _moveDirection = new Vector3(_networkedMoveDirection.x, 0, _networkedMoveDirection.y); }
